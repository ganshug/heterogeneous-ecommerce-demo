...
#                          Keep this file locally (e.g. db2-dashdb.lic) — it is gitignored
#
# Usage:
#   # Option 1: Environment variables
#   export IBM_ENTITLEMENT_KEY="<your-key>"
#   export DB2_LICENSE_FILE="./db2-dashdb.lic"
#   bash deploy.sh
#
...
if [[ -z "${DB2_LICENSE_FILE:-}" ]]; then
  # Check if local db2-dashdb.lic exists (default location)
  if [[ -f "${SCRIPT_DIR}/db2-dashdb.lic" ]]; then
    DB2_LICENSE_FILE="${SCRIPT_DIR}/db2-dashdb.lic"
    info "Found local license file: ${DB2_LICENSE_FILE}"
  else
    echo ""
    ...
    echo "  Tip: Place it as db2-dashdb.lic in this directory (it is gitignored)."
    echo ""
